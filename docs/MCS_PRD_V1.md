# 物料编码系统 PRD（V1.4 BOM审核轻量归档版）

> **版本说明（V1.4）**：在不破坏 V1.3 冻结功能（手工建料、替代料、搜索、导出、数据库备份恢复）的前提下，新增 **BOM 导入审核能力（轻量归档版）**：上传 BOM → 自动分析 → 新料导入 → 无异常后归档保存原文件；不拆明细入库，仅新增轻量索引表用于历史查询。
>
...
* 【V1.4 新增】BOM Excel 导入
* 【V1.4 新增】BOM 自动审核
* 【V1.4 新增】BOM 新料识别与导入
* 【V1.4 新增】BOM 审核通过后归档保存
* 【V1.4 新增】BOM 历史记录查询（轻量索引）
...
本节定义 V1.4 的 **BOM Excel 导入审核（轻量归档版）**。能力边界强调：

* **上传 BOM → 自动分析 → 新料导入 → 无异常后归档保存原文件**
* **审核通过后不拆 BOM 明细入库**（仅保存原始 Excel 文件 + 一张轻量索引表用于历史查询）
* **不改变 V1.3 任何既有功能与手工建料逻辑**
* **V1.4 不启用 BOM 场景自动编码建料**：BOM 明细行 `code` 为空时，判定为异常，仅展示，不允许在本模块内生成编码或导入建料（见 7.7.3）

#### 7.7.0 禁止事项（必须写入）

V1.4 BOM 审核模块禁止以下行为（必须写死，避免歧义）：

1. 根据 **空编码（code 为空）** 自动生成新编码建料（V1.4 不启用 BOM 场景自动编码建料）
2. 在 BOM 页面内手工输入/编辑编码以“修复”异常行（异常仅展示，需用户线下修复源 Excel）
3. 自动修改 BOM 原文件内容（包括但不限于：补列、改单元格值、重写/另存为、改变格式）
...
* 审核对象为 **BOM 明细行**（每行对应一个待比对物料条目）
* 审核结果分为：`PASS` / `NEW` / `ERROR`（见 7.7.4）
* V1.4 只处理“物料维度”审核，不处理数量/位号/用量等工程维度校验（见 7.7.3）

#### 7.7.2 BOM 文件格式定义（必须写入）

参考项目内模板文件：`docs/template/BOM.xls`。
...
* `code` **非空**
* `code` 在物料库存在（可定位到 `material_item.code`）
* `code` 对应物料必须为 **启用态（status=1）**（若该 `code` 对应记录为 `status=0` 已废弃，则不得判定为 PASS）
* `spec` 在物料库存在（可定位到某条 `material_item.spec`；是否启用态以物料库既有规则为准）
* `code/spec` 对应一致（BOM 行 `spec` 与库内该 `code` 的 `spec` 一致）
...
* `code` **非空**
* `code` 不存在（物料库中不存在该 `code`）
* `spec` 不存在（物料库中不存在该 `spec` 在 **启用态(status=1)** 数据集的重复占用；见 10.1 口径）
...
* 管理员可对单行点击：`导入物料库`
* 允许批量动作：`一键导入全部新料`
* 导入成功后必须触发 **重新审核该行状态**（NEW → PASS 或 ERROR）

##### 7.7.3.3 异常物料（仅展示，不允许页面内修复）

以下任一成立即为异常（必须分型展示“异常原因”，用于用户定位 Excel 源问题）：

**优先级（必须明确，避免歧义）**

* 若同时满足多种异常类型，**类型E（物料已废弃）优先展示**（用于明确“为何该编码存在但仍不可用”）。
...
**类型B：编码与规格冲突（code 存在但 spec 已存在且对应关系不一致）**

* 条件：
  * `code` 在物料库存在
  * `spec` 在物料库存在
  * 且 BOM 行 `code/spec` 对应关系不一致（BOM 行 `spec` ≠ 库内该 `code` 的 `spec`）
* 提示：编码与规格冲突
...
* 条件：
  * `code` 在物料库存在
  * 且 BOM 行 `spec` 与库内该 `code` 的 `spec` 不一致
* 提示：编码已存在但规格不一致
...
* 条件：
  * `code` 不存在（物料库中不存在该 `code`）
  * 但 `spec` 已存在（在同分类启用态数据集中已被占用的规格号）
* 提示：规格已存在，疑似重复建料
...
* V1.4 异常物料 **仅展示**（用于人工线下修复 Excel/数据源），**不允许在 BOM 页面内修复**
* V1.4 不提供“自动纠错/建议替代料/差异比对”等能力（见第三章“不包含”）
* V1.4 不允许“空编码行作为 NEW 导入”，也不允许在 BOM 流程中为其生成编码（见 7.7.0）

#### 7.7.4 保存规则（必须明确）

按钮：`[保存审核通过BOM]`

仅当以下条件全部满足时，允许显示按钮（隐藏或禁用二选一，但语义必须一致）：
...
#### 7.7.5 归档保存规则（重点，必须写入）

审核通过后，**不拆 BOM 明细入库**。

归档保存内容：

* 直接保存用户上传的 **原始 Excel 文件**
...
```text
{BOM归档目录}/BOM/{成品编码}/{PCBA版本}.xls
```

示例：

```text
BOM/CP00A1062A/KC001_RS_MB_V1.2_A_V1.0-20260414.xls
```

归档目录配置（体验增强版，必须明确）：

* 默认：未配置时按 **程序执行目录** 作为归档目录；
* 首次点击 `[保存审核通过BOM]` 且未配置归档目录时：必须弹出对话框让用户选择一次归档目录；
* 选择后需写入偏好配置（建议键：`BomArchiveRootPath`），后续默认使用该目录；
* 必须提供“更改归档目录”的入口。
...
* 必须弹出确认：该版本已存在，是否覆盖保存？
* 弹窗需展示（若可取）：原保存时间、原路径
* 若用户确认覆盖：
  * 覆盖目标文件（必须保持“原文件原封不动”归档原则；覆盖过程需满足“临时文件 + 原子替换”且不得残留 tmp）
  * 更新 `bom_archive.file_path`
  * 更新 `bom_archive.created_at` 为当前时间
* 若用户取消：终止保存，不得变更任何 DB 记录与文件

#### 7.7.7 数据库设计（轻量版，必须写入）

新增索引表（仅用于历史版本查询与快速定位文件，**不保存 BOM 明细行数据**）：
...
| --- | --- | --- |
| id | int | 主键 |
| finished_code | string | 成品编码 |
| version | string | PCBA版本号（完整，含末尾日期时间） |
| file_path | string | 归档文件路径（相对或绝对需在实现中统一） |
...
`description` 字段在以下场景统一调用现有标准化模块：
...
* 统一走公共 Normalize 模块，**禁止 BOM 单独写逻辑**
...
* 选择文件（仅 Excel；模板参考 `docs/template/BOM.xls`）
* 显示：成品编码（从表头解析）
* 显示：版本号（从表头解析）
* 统计区：总数 / 通过(PASS) / 异常(ERROR) / 新料(NEW) / **异常物料（缺编码）**
  * 说明：当 BOM 中存在大量空编码物料时，必须单独显示“异常物料（缺编码）：X 条”，便于用户快速识别来源文件问题（见 7.7.3 类型A）

列表字段（列固定，建议顺序；本版本要求体现“异常原因”）：
...
* 当状态为 `NEW`：显示 `导入物料库`
* 当状态为 `ERROR`：无导入按钮（操作列为空）
* 当 `code` 为空：操作列为空（必为 ERROR 类型A）
* 当状态为 `PASS`：无操作按钮（操作列为空）

按钮（必须）：

* `[一键导入全部新料]`（仅当存在 NEW 时可用）
* `[保存审核通过BOM]`（仅当 NEW=0 且 ERROR=0 时显示或可用，见 7.7.4）
...
* 本节定义的 `Normalize` 视为系统内 **公共 Normalize 模块**（禁止各业务模块各写一套），至少覆盖以下场景：
  * 手工建料
  * BOM 导入审核（V1.4）

V1.4 增量规则（必须先做，且仅此一条）：

* 将 `description` 中的中文逗号（`，`）统一替换为英文逗号（`,`）
